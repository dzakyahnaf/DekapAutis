-- Gives the demo child development focuses the rest of the system recognises.
--
-- The seed stored fokus_perkembangan as array['komunikasi', 'kemandirian'] -
-- category names. Both readers expect focus keys instead:
--
--   * the client parses with FokusPerkembangan.fromDb, which drops unknown
--     values silently, so Bima's profile showed no focus at all
--   * generate-plan looks each value up in FOKUS_KE_KATEGORI, found nothing,
--     and built "Susun rencana" plans that ignored his focus entirely
--
-- komunikasi_ekspresif and rutinitas_pagi map to the same two categories the
-- seed meant, and rutinitas_pagi matches the morning-routine question the demo
-- asks Tanya Dekap. The body is otherwise identical to 20260919010016.
...
begin
  ...
  update profil_anak set
    ...
    -- Keys of FOKUS_KE_KATEGORI in _shared/pemetaan.ts, not category names.
    fokus_perkembangan    = array['komunikasi_ekspresif', 'rutinitas_pagi']
  where id = v_anak;
